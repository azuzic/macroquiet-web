...
<template>
    <div class="grow flex flex-col justify-between items-start pt-16 absolute overflow-y-auto">
        <div class="flex grow pb-96 flex-col | justify-center items-center bg-red | bg-opacity-30 p-2 | w-full">
            ...
            <MQ_h2 class="w-full" text="Change username" :small="true"/>
            ...
            <Form v-if="this.$route.params.token != 'enter-email'" @submit="onSubmit" class="w-full max-w-md flex justify-between px-12 sm:px-6 | transition-all duration-500" :validation-schema="schema">
                <div class="flex flex-col w-full justify-between items-center gap-2">
                    ...
                    <MQ_textInput label="New Username" icon="user" :max="24"/>
...
export default {
    ...
    setup() {
        ...
        const schema = yup.object({
            "New Username":  yup.string().required().label("Username")
                .matches(/^[^!@?#$%\/\-^"&*\\+=<>:;|~\s]*$/, { message: 'No symbols' })
                .test(str => str.indexOf(' ') == -1),
        });

        return {
            schema, globalStore, macroquiet_logo_icon
        };
    },
    data() {
        return {
    ...
    methods: {
        async onSubmit(values) {
            if (values) {
                ...
                try {
                    await wait(2);
                    this.response = await User.putChangeUsername({ new_username: values['New Username'] });
                    ...
                    await wait(3);
                    this.type = "none";
                    console.error({message: "Error sending message!", error: e});
                }
            }
        },
    },
