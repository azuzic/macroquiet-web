<template>
    <div class="w-full flex gap-2 items-center drop-shadow-MQ z-20">
        ...
        <div class="relative z-20 w-full mb-6 group">
            <Field type="text" :name="label" :rules="rules" v-slot="{ field, errorMessage }" v-model="value">
                ...
                <svg class="w-4 h-4 absolute top-4 right-0 text-MQ_light text-opacity-60 peer-focus:text-MQ_light transition-all duration-300 cursor-pointer -z-10" :class="show ? '-scale-100' : ''" aria-hidden="true" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                ...
                <label v-else class="peer-focus:font-medium absolute text-MQ_light text-opacity-60 peer-focus:text-MQ_light text-sm transform top-3 transition-all duration-300 -z-10 origin-[0] left-0 placeholder-shown:scale-100 placeholder-shown:translate-y-0 scale-75 -translate-y-6">
                    {{ label }} 
                </label>

                <div @mouseleave="focus = false" @mouseenter="focus = true" :class="show ? 'h-fit max-h-64 opacity-100' : 'h-0 opacity-0'"
                    class="w-full mt-4 | bg-MQ_dark bg-opacity-90 | drop-shadow-md rounded-lg | overflow-hidden | transition-opacity duration-300 | absolute overflow-y-auto | z-50">
