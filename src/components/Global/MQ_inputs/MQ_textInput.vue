    ...
    <div class="w-full flex gap-2 items-center drop-shadow-MQ">
        ...
        <div class="relative z-0 w-full mb-6 group">
            <Field :name="label" :rules="rules" v-slot="{ field, errorMessage }">
                ...
                <div v-if="max" class="absolute w-full text-right text-xs text-MQ_red mt-1 font-bold " :class="errorMessage ? 'text-MQ_red' : 'text-MQ_light text-opacity-75'"> {{ field.value ? field.value.length : 0 }} / {{ max }} </div>
